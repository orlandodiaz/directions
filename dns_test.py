...
def dnsquery(domain):
    ...
    l1 = "\x41\x41"  # AA   Arbitrary ID
    l2 = "\x01\x00"  # 0  Query params: 0 for query, 1 for resp
    l3 = "\x00\x01"  # 1  Number of questions
    l4 = "\x00\x00"  # 0 0  Number of answers
    l5 = "\x00\x00"  # 0 0  Number of authority records
    l6 = "\x00\x00"  # 0 0  Number of additional records

    header = l1 + l2 + l3 + l4 + l5 + l6

    # Question section
    q = d + "\x00\x00\x01\x00\x01"  # 0 0 1 0 1
    m = header + q
...
def Conn(ser, q):
    ...
    if (r == 0):  # no error, get the ip address
        ...
        # error
        print
        "*** None exist domains: server can not find answer."
    elif (r == 4):
        # error
        print
